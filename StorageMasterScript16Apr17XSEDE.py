...
################################################################################
def setKeyParameters(scenario,co2CapScenario,startYear,endYear,stoScenarios):
    #KEY PARAMETERS
    runLoc = 'remote' #'pc' or 'remote'. If remote, assumes all data is in local dir. 
    xsedeRun = True #whether running on XSEDE supercomputer (affects sys_dir in GAMS call)
    #Scenarios:solar & nuclear = low cap costs, ng = low fuel price, coalret = early coal rets, highSto = more sto
    # scenario = 'cpp' #cpp, deep, solar, nuclear, ng, coalret, highSto, base, highStoEff, noco2price
    # co2CapScenario = 'cpp' #cpp, deep, or none (deep = 80% redux from 1990 by 2050)
    calculateCO2Price = True
    runCE,runUC = False,True
    yearStepCE = 10
    daysPerSeason = 1
    ...
    # stoScenarios = ['NoSto','StoEnergy'] #['NoSto','StoEnergy','StoRes','StoEnergyAndRes']
...
###### MASTER FUNCTION #########################################################
################################################################################
def masterFunction(scenario,co2CapScenario,startYear,endYear,stoScenarios):
    (runLoc,testModel,states,statesAbbrev,powerSystems,compressFleet,fuelPricesTimeSeries,
        ...
        runFirstUCYear,demandYear,firstUCYear,ptEligRetCF,incITC) = setKeyParameters(scenario,
        co2CapScenario,startYear,endYear,stoScenarios)
    ...
    for currYear in range(startYear,endYear,yearStepCE):
        ...
        if currYear > firstUCYear and runCE == True:
            ...
            (genFleet,genFleetNoRetiredUnits,genFleetPriorCE,priorCapacExpModel,
                ...
                fuelPricesTimeSeries,states,statesAbbrev,scaleMWtoGW,scaleDollarsToThousands,
                ...
                genFleetPriorCE,priorCapacExpModel,priorHoursCE,incITC)
        if runUC == True:
...
def runCapacityExpansion(genFleet,demandWithGrowth,startYear,currYear,endYear,
        ...
        regErrorPercentile,flexErrorPercentile,rrToRegTime,rrToFlexTime,rrToContTime,
        regUpCostCoeffs,xsedeRun,scenario,ptEligRetCF,genFleetPriorCE,priorCapacExpModel,
        priorHoursCE,incITC):
    ...
    if priorCapacExpModel != None: #if not in first CE loop
        unitsRetireCFPriorCE = retireUnitsCFPriorCE(genFleet,genFleetPriorCE,retirementCFCutoff,
            priorCapacExpModel,priorHoursCE,scaleMWtoGW,ptEligRetCF,currYear)
